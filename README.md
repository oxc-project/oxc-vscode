...
| `oxc.runExternalCode`         | `false`       | `true` \| `false`                                                                                             | Allow type-aware linting to run TypeScript `contentMappers`, needed for files that are not TypeScript, like Ember's `.gts`. Requires type-aware linting. This executes code from your project, so it is ignored in untrusted workspaces and cannot be enabled from a configuration file. |
