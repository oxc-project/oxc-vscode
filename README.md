...
| `oxc.lint.rules.customizations` | `[]`          | `<array>`                                                                                                     | Override diagnostic severities reported by the linter. Each entry maps a rule pattern to a new severity. - Use `"*"` as a wildcard to match all rules. - `"downgrade"` reduces error to warning, and warning to info. This is useful to keep red squiggly lines for TypeScript errors only. - `"default"` resets a previously matched rule back to its original severity. |
